...
class Sub_category_model extends CI_Model {
	...
	function get_records_json($category_id)
	{
		if((int)$category_id == 0) return '[]';
		$query = $this->db->query('select SUB_CAT_ID,NAME from sub_categories WHERE CATEGORY_ID = ?;',$category_id);
		if($query->num_rows() > 0) return json_encode($query->result());
		return '[]';
	}
